	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			fclose($file);
      			...
      			// send mail to lbit
			if (isset($this->request->post['pubblicazione'])) {
				$pubblicazione = $this->request->post['pubblicazione'] ;
			}
			else {
				$pubblicazione=0;
         }
         ...
         $header = array("From: OpenCart LBiT <no-replay@$host>",
                     "Reply-To:OpenCart LBiT webmaster<info@$host>",
                      "Content-Type: text/html"
                      );
         $headers = implode("\r\n",$header) . "\r\n";
         $message = $host.';'.$host.''.$uri.';'.$pubblicazione.';3.0.2.0;';                       
         $a="newinstall@tuttoperilpc.it";
         $oggetto="OpenCart LBiT $host";
         mail($a, $oggetto, $message, $headers);      
         // end send mail to lbit
